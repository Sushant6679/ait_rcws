...
// Changes made here
void (*TF_WriteCallback)(const uint8_t *buff, uint32_t len) = NULL;  // Callback pointer

void TF_WriteImpl(TinyFrame *tf, const uint8_t *buff, uint32_t len) {
    if (TF_WriteCallback) {
        TF_WriteCallback(buff, len);  // Call the Python-provided function
    } else {
        // Provide a default behavior (e.g., print an error)
        printf("Error: TF_WriteCallback not set!\n");
    }
}

// Function to allow Python to set the callback
void TF_SetWriteCallback(void (*callback)(const uint8_t *buff, uint32_t len)) {
    TF_WriteCallback = callback;
}
